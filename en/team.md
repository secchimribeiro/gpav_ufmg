...
{% for m in members %}
  <article class="card">
    {% if m.photo %}<img class="card-photo" src="{{ site.baseurl }}{{ m.photo }}" alt="Photo of {{ m.name }}">{% endif %}
    <div class="card-body">
      <h3 class="card-title">{{ m.name }}</h3>
      {% if m.title %}<p class="card-subtitle">{{ m.title }}</p>{% endif %}
      <p>{{ m.bio_en }}</p>
      {% if m.linkedin %}<p><a href="{{ m.linkedin }}" aria-label="LinkedIn of {{ m.name }}">LinkedIn</a></p>{% endif %}
    </div>
  </article>
{% endfor %}
</div>
